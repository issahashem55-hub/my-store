const CACHE="abu-akbar-store-final3";
...
self.addEventListener("fetch",e=>{
  ...
  if(u.pathname.endsWith("/apps.json")){
    e.respondWith(fetch(e.request,{cache:"no-store"}).then(r=>{
      const c=r.clone(); caches.open(CACHE).then(x=>x.put("./apps.json",c)); return r;
    }).catch(()=>caches.match("./apps.json")));
    return;
  }
  e.respondWith(caches.match(e.request).then(c=>c||fetch(e.request).then(r=>{
    if(r.ok){const c2=r.clone();caches.open(CACHE).then(x=>x.put(e.request,c2));}
    return r;
